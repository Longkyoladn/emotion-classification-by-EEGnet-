...
def main():
 p=argparse.ArgumentParser(); p.add_argument("--dataset-root",type=Path,required=True); p.add_argument("--manifest",type=Path,required=True); p.add_argument("--ica",type=Path,required=True); p.add_argument("--subject",default="sub-mit003"); p.add_argument("--processed-dir",type=Path,default=Path("data/processed")); p.add_argument("--qc-dir",type=Path,default=Path("reports/qc")); p.add_argument("--bad-channel-decisions",type=Path); p.add_argument("--ica-decision",type=Path); a=p.parse_args()
 decisions_path=a.bad_channel_decisions or a.qc_dir/a.subject/"p4_bad_channel_decisions.csv"; decisions=pd.read_csv(decisions_path); bads=decisions.loc[decisions.decision.eq("bad"),"channel"].tolist()
 ica_decision_path=a.ica_decision or a.qc_dir/a.subject/"p6b_ica_decision.json"; ica_decision=json.loads(ica_decision_path.read_text(encoding="utf-8")); excluded_components=[int(x) for x in ica_decision["selected_components"]]
 ...
 for tr in ts.itertuples(index=False):
  ...
  ecg=seg.get_data(picks=["ECG"])[0]; eeg=seg.copy().pick(EEG); eeg.info["bads"]=bads
  if bads: eeg.interpolate_bads(reset_bads=False,method={"eeg":"spline"},verbose="ERROR")
  eeg.set_eeg_reference("average",projection=False,verbose="ERROR"); ica.apply(eeg,exclude=excluded_components,verbose="ERROR")
