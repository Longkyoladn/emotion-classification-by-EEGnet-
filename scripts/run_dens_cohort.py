...
STEPS = [
    ...
    (
        ...
        lambda s: [
            ...
            "data/manifest/dens_trials.csv",
            "--ica",
            f"reports/qc/{s}/p6_ica_solution-ica.fif",
            "--subject",
            s,
        ],
...
def materialized(root: Path, subject: str) -> bool:
    files = list((root / subject / "eeg").glob("*_eeg.set")) + list(
        (root / subject / "eeg").glob("*_eeg.fdt")
    )
...
def main():
    ...
    a = p.parse_args()
    root = Path("data/raw/ds003751")
    table = pd.read_csv("data/manifest/dens_subjects.csv")
    eligible = table[
    ...
    summary = {
        ...
        "fully_curated": int(
            sum(
                (Path("reports/qc") / s / "p10_curation_summary.json").exists()
                for s in subjects
            )
        ),
