...
def main() -> None:
    ...
    decisions_path = (
        args.bad_channel_decisions
        or args.output_dir / args.subject / "p4_bad_channel_decisions.csv"
    )
    ...
    for tr in trials.itertuples(index=False):
        start, stop = int(round(tr.onset_seconds * sfreq)), int(
            round(tr.end_seconds * sfreq)
        )
        ps, pe = max(0, start - pad), min(raw.n_times, stop + pad)
        seg = (
            raw.copy()
            .crop(ps / sfreq, (pe - 1) / sfreq, include_tmax=True)
            .load_data(verbose="ERROR")
        )
        seg.notch_filter([50.0], phase="zero", verbose="ERROR").filter(
            1.0, 45.0, phase="zero", verbose="ERROR"
        )
    ...
    ica = mne.preprocessing.ICA(
        n_components=0.99, method="fastica", random_state=42, max_iter=1000
    )
    ...
    for i, src in enumerate(sources):
        ...
        low = np.trapezoid(power[(f >= 1) & (f < 4)], f[(f >= 1) & (f < 4)]) / max(
            total, 1e-20
        )
        high = np.trapezoid(
            power[(f >= 30) & (f <= 45)], f[(f >= 30) & (f <= 45)]
        ) / max(total, 1e-20)
        kurt = float(pd.Series(src).kurt())
        reasons = []
        if abs(corrs[0]) >= 0.3:
            reasons.append("ecg_correlation")
        if max(abs(corrs[1]), abs(corrs[2])) >= 0.3:
            reasons.append("emg_correlation")
        ...
        if high >= 0.40 and abs(kurt) >= 15:
            reasons.append("high_frequency_power_with_kurtosis")
        rows.append(
            {
                "component": i,
                "ecg_correlation": corrs[0],
                "emg_correlation": corrs[1],
                "emg2_correlation": corrs[2],
                "low_frequency_fraction": low,
                "high_frequency_fraction": high,
                "kurtosis": kurt,
                "proposed_artifact": bool(reasons),
    ...
    if proposed:
        ...
        for j, fig in enumerate(figs):
            fig.savefig(out / f"p6_proposed_topographies_{j+1}.png", dpi=150)
            plt.close(fig)
        fig, axes = plt.subplots(
            len(proposed), 1, figsize=(10, max(3, 2.2 * len(proposed))), squeeze=False
        )
        for ax, idx in zip(axes[:, 0], proposed):
            f, pw = welch(sources[idx], fs=sfreq, nperseg=int(4 * sfreq))
            mask = (f >= 1) & (f <= 45)
            ax.semilogy(f[mask], pw[mask])
            ax.set(
                title=f"IC {idx}: {table.loc[idx,'reasons']}", xlabel="Hz", ylabel="PSD"
            )
            ax.grid(alpha=0.25)
        fig.tight_layout()
        fig.savefig(out / "p6_proposed_component_spectra.png", dpi=150)
        plt.close(fig)
    ...
    report = {
        ...
        "review_candidates": proposed,
        "proposal_thresholds": {
            "absolute_ecg_correlation": 0.3,
            "absolute_emg_correlation": 0.3,
            "high_frequency_fraction_with_absolute_kurtosis": {
                "high_frequency_fraction": 0.40,
                "absolute_kurtosis": 15,
            },
            "kurtosis_alone_is_not_a_rule": True,
        },
        "ica_applied_to_eeg": False,
    ...
    (out / "p6_ica_summary.json").write_text(
