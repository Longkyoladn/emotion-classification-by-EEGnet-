...
def main() -> None:
    ...
    parser.add_argument(
        "--subjects", type=Path, default=Path("data/manifest/dens_subjects.csv")
    )
    parser.add_argument("--exclude", nargs="*", default=["sub-mit003"])
    parser.add_argument("--include", nargs="*")
    parser.add_argument(
        "--extensions", nargs="+", choices=["set", "fdt"], default=["set", "fdt"]
    )
    ...
    if args.all_subject_directories:
        eligible = sorted(
            path.name for path in args.dataset_root.glob("sub-*") if path.is_dir()
        )
    ...
    for subject in eligible:
        ...
        for extension in args.extensions:
            files += sorted(
                (args.dataset_root / subject / "eeg").glob(f"*_eeg.{extension}")
            )
        if len(files) != len(args.extensions):
            raise RuntimeError(
                f"Expected {len(args.extensions)} selected EEG files for {subject}, "
                f"found {len(files)}"
            )
        for path in files:
