...
def main() -> None:
    ...
    metrics, validation = [], []
    waveforms = {
        state: [] for state in ["filtered", "interpolated", "average_referenced"]
    }

    for trial in trials.itertuples(index=False):
        start, stop = int(round(trial.onset_seconds * sfreq)), int(
            round(trial.end_seconds * sfreq)
        )
        pstart, pstop = max(0, start - pad), min(raw.n_times, stop + pad)
        segment = (
            raw.copy()
            .crop(pstart / sfreq, (pstop - 1) / sfreq, include_tmax=True)
            .pick(CHANNELS)
            .load_data(verbose="ERROR")
        )
        segment.notch_filter([50.0], phase="zero", verbose="ERROR").filter(
            0.5, 45.0, phase="zero", verbose="ERROR"
        )
        segment.crop(
            (start - pstart) / sfreq, (stop - pstart - 1) / sfreq, include_tmax=True
        )
        ...
        referenced = interpolated.copy().set_eeg_reference(
            "average", projection=False, verbose="ERROR"
        )

        arrays = {
            "filtered": filtered.get_data() * 1e6,
            "interpolated": interpolated.get_data() * 1e6,
            "average_referenced": referenced.get_data() * 1e6,
        }
        for state, x in arrays.items():
            ...
            for channel in BAD_CHANNELS:
                ...
                metrics.append(
                    {
                        "trial": int(trial.event_trial_index),
                        "state": state,
                        "channel": channel,
                        "std_uv": np.std(x[i]),
                        "peak_to_peak_uv": np.ptp(x[i]),
                        "median_correlation_to_others": np.median(
                            [np.corrcoef(x[i], x[j])[0, 1] for j in others]
                        ),
                        "common_mode_rms_uv": np.sqrt(np.mean(common_mode**2)),
                    }
                )
            ...
            validation.append(
                {
                    ...
                    "nrmse": nrmse,
                    **{
                        f"{b}_relative_error": abs(estimate_bp[b] - true_bp[b])
                        / max(true_bp[b], 1e-20)
                        for b in BANDS
                    },
                }
    ...
    summary = {
        ...
        "good_channel_validation": {
            ...
            "q25_correlation": float(validation_df.correlation.quantile(0.25)),
            "median_nrmse": float(validation_df.nrmse.median()),
            "q75_nrmse": float(validation_df.nrmse.quantile(0.75)),
            "median_band_power_relative_error": {
                b: float(validation_df[f"{b}_relative_error"].median()) for b in BANDS
            },
        },
        "interpolation_effect": {
            "max_absolute_change_on_126_good_channels_uv": float(
                np.max(np.abs(good_difference))
            ),
            "E50_median_std_before_uv": float(
                metric_df[
                    (metric_df.channel == "E50") & (metric_df.state == "filtered")
                ].std_uv.median()
            ),
            "E50_median_std_after_uv": float(
                metric_df[
                    (metric_df.channel == "E50") & (metric_df.state == "interpolated")
                ].std_uv.median()
            ),
            "E103_median_std_before_uv": float(
                metric_df[
                    (metric_df.channel == "E103") & (metric_df.state == "filtered")
                ].std_uv.median()
            ),
            "E103_median_std_after_uv": float(
                metric_df[
                    (metric_df.channel == "E103") & (metric_df.state == "interpolated")
                ].std_uv.median()
            ),
        },
        "average_reference_effect": {
            "common_mode_rms_before_uv": float(common_before),
            "common_mode_rms_after_uv": float(common_after),
        },
    ...
    (out / "p5_summary.json").write_text(
    ...
    for ax, channel in zip(axes, BAD_CHANNELS):
        for state in ["filtered", "interpolated", "average_referenced"]:
            values = metric_df[
                (metric_df.channel == channel) & (metric_df.state == state)
            ].std_uv
            ax.plot(range(1, len(values) + 1), values, marker="o", label=state)
        ax.set(title=f"{channel}: std by trial", xlabel="Trial", ylabel="Std (µV)")
        ax.grid(alpha=0.25)
        ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(out / "p5_bad_channel_std_comparison.png", dpi=160)
    plt.close(fig)
