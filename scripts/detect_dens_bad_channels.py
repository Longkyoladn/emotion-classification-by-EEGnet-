...
def main() -> None:
    ...
    for trial in trials.itertuples(index=False):
        ...
        x = mne.filter.notch_filter(
            x, Fs=sfreq, freqs=[50.0], phase="zero", verbose="ERROR"
        )
        ...
        neighbor_corr = np.array(
            [np.nanmedian(corr[i, neighbors[i]]) for i in range(128)]
        )
        ...
        for i, channel in enumerate(CHANNELS):
            ...
            if z_global[i] < -3.5:
                flags.append("global_correlation")
            if z_neighbor[i] < -3.5:
                flags.append("neighbor_correlation")
            trial_metrics.append(
                {
                    "trial": int(trial.event_trial_index),
                    "channel": channel,
                    "std_uv": std[i],
                    "peak_to_peak_uv": p2p[i],
                    "global_correlation": global_corr[i],
                    "neighbor_correlation": neighbor_corr[i],
                    "flags": ";".join(flags),
                    "flagged": bool(flags),
                }
            )
        ...
        rows.append(
            {
                ...
                "trial_fraction": flagged_fraction,
                ...
                "neighbor_correlation_trials": int(
                    counts.get("neighbor_correlation", 0)
                ),
                ...
                "median_neighbor_correlation": float(
                    group.neighbor_correlation.median()
                ),
            }
        )
    ...
    (out / "p4_bad_channel_summary.json").write_text(
